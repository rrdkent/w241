...
This is a model that has *only* an intercept, which is indicated in the `1` in the formula call. Note also that just as we are able to perform relatively simple functions on the column space using `data.table` (e.g. `d[ , mean(x)]`) we can also peform relatively more complex functions -- like a `t.test` or a `lm`. Neato. 
