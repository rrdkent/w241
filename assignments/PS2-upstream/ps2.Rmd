...
**For each state, we calculate the standard error of the estimated ATE by using the following equation, number 3.4 in the FE:**

$SE(\hat{ATE})=\sqrt{\frac{1}{N-1}\left\{ \frac{mVar(Y_i(0))}{N-m}+\frac{(N-m)Var(Y_i(1))}{m}+2Cov(Y_i(0),Y_i(1)) \right\} }$


```{r}
cov((dt[which(texas0_arkansas1==1 & term2year==0),,]$bills_introduced),(dt[which(texas0_arkansas1==1 & term2year==1),,]$bills_introduced))
```
...
**And we calculate ** $\hat{SE} = \sqrt{\frac{\hat{Var(Y_i(0))}}{N-m}+\frac{\hat{Var(Y_i(1))}}{m}}$
...
**Pooling the data from the two states introduced by two-year senators compared to four-year senators leads to a biased estimate of the overall ATE chiefly because each state has different treatment effects and **
...
```{r}
SE_ATE <- sqrt((AR_SE)^2*(AR_Y1_n+AR_Y0_n/tot_obs)^2+(AR_TX)^2*(TX_Y1_n+TX_Y0_n/tot_obs)^2)
SE_ATE
```
...
run_study <- function(state){
  # pull a sample & store it in a vector.  I'm choosing samples of 10.
  my_sample <- as.data.table(sapply((dt[which(texas0_arkansas1==state),,]), sample, 10))
  #calculate ate-hat by subtracting sample mean loss from mean success
  ate <- my_sample[,.(mean = mean(bills_introduced)),by = term2year]$mean[2] - my_sample[,.(mean = mean(bills_introduced)),by = term2year]$mean[1]
  #return ate-hat
  ate
  #my_sample
}
...
```{r}
TX_true_ATE <- dt[which(texas0_arkansas1==0 & term2year==1),.(mean(bills_introduced)),] - dt[which(texas0_arkansas1==0 & term2year==0),.(mean(bills_introduced)),]
TX_true_ATE
```

```{r}
AR_true_ATE <- dt[which(texas0_arkansas1==1 & term2year==1),.(mean(bills_introduced)),] - dt[which(texas0_arkansas1==1 & term2year==0),.(mean(bills_introduced)),]
AR_true_ATE
```
...
```{r}
TX_p_val <-mean(TX_true_ATE < TX_results )*2
TX_p_val
```

```{r}
AR_p_val <-mean(AR_true_ATE < AR_results )*2
AR_p_val
```
...
```{r} 
#There are fourteen observations, so by matching them, we will get 7 clusters.  We will randomly select 3 clusters to get treatment and 3 to be the control.  One will be held-out because covariance must be calculated with similar-size matricies.
...
```{r}
#Similar to above, there are fourteen observations, so by matching them, we will get 7 clusters.  We will randomly select 3 clusters to get treatment and 3 to be the control.  One will be held-out because covariance must be calculated with similar-size matricies.
...
**If shown to all users, at the current metrics, the revenue is 500K.  To calculate profit, we subtract cost (adspend) from revenue, that is, 500K-100K is 400K profit. **

**To calculate breakeven, let' set up an algebra equation**

$users*conversion*unit_profit-(cpi * users)=0$
$users*.005*100-(.1*users)=0$
$users*.5-(.1*users)=0$
$.5*users=.1 users$ 
...
t.test(treat_mean ~ control_mean, mu=0, alt="two.sided", conf=0.95)

---------
#convert to datatable, take the datatable, where unform price auction is one

dt <-data.table(d2)
treat_vector <- dt[uniform_price_auction == 1,,]$bid

#and now the non-uniform price auction (control):

control_vector <- dt[uniform_price_auction == 0,,]$bid
...
t_test <- t.test(dt[,,]$bid ~ dt[,,]$uniform_price_auction, mu=0, alt="two.sided", conf=0.95)

t_test
```

```{r}
fit <- lm(dt[,,]$bid ~ dt[,,]$uniform_price_auction)
fit_sum <- summary(fit)
```
...
```{r}
confint(fit, dt[,,]$uniform_price_auction, level=0.95)
```
...
**Now let's calculate the true ATE for this sample:**
...
**Well, this is somewhat confusing.  Because our true-ATE has a negative average treatment effect of approximately -12.21, versus our mean treatment effect for our randomized inference sampling is approximately positive .34.**

**We may now ask ourselves, how many of the simulated random assignments generate an estimated ATE that is at least as large as the actual estimate of the ATE? **
...
**This is an insanely high one-tailed p-value.  What's going on?  It's going to be nuts to calculate the two-tailed p-value.**
